...
def assemble_words_db():
    ...
    try:
        ...
        and os.path.exists(profile.data['words_db_file']):
            os.remove(profile.data['words_db_file'])

        con = sqlite3.connect(profile.data['words_db_file'])
    ...
    if create_words_db(con, cur):
        print 'Successfully Created '+profile.data['words_db_file']
